...
/*
================
rvWeaponMachinegun::OnLaunchProjectile
================
*/
...
/*
================
rvWeaponMachinegun::DrumSpin

Set the drum spin speed
================
*/
bool rvWeaponMachinegun::DrumSpin(int speed, int blendFrames) {
...
STATE("DrumSpinDown", rvWeaponMachinegun::State_DrumSpinDown)
STATE("DrumSpinUp", rvWeaponMachinegun::State_DrumSpinUp)

STATE("ClaspOpen", rvWeaponMachinegun::Frame_ClaspOpen)
STATE("ClaspClose", rvWeaponMachinegun::Frame_ClaspClose)
END_CLASS_STATES
...
stateResult_t rvWeaponMachinegun::State_Idle(const stateParms_t& parms) {
	...
	case STAGE_WAIT:
		...
		if (!clipSize) {
			...
			if (wsfl.attack && AutoReload()) {
				SetState("Reload", 4);
				return SRESULT_DONE;
			}
			if (wsfl.netReload || (wsfl.reload)) {
				SetState("Reload", 4);
				return SRESULT_DONE;
			}
		}
...
stateResult_t rvWeaponMachinegun::State_Fire(const stateParms_t& parms) {
	...
	case STAGE_FIRE:
		...
		if (wsfl.zoom) {
			if (!gameLocal.GetLocalPlayer()->ability2Upgraded){
				static_cast<idPhysics_Player *>(gameLocal.GetLocalPlayer()->GetPlayerPhysics())->SetLinearVelocity(static_cast<idPhysics_Player *>(gameLocal.GetLocalPlayer()->GetPlayerPhysics())->GetLinearVelocity() / 300.0f);
			}
			...
			gameLocal.GetLocalPlayer()->SetSpeed(0.0f);
	...
	case STAGE_DONE:
		gameLocal.GetLocalPlayer()->inventory.armor -= 50 +25 * gameLocal.GetLocalPlayer()->ability2Upgraded;
		...
		if (!wsfl.attack && AutoReload() && !wsfl.lowerWeapon) {
			PostState("Reload", 4);
		}
		else {
			PostState("Idle", 4);
...
/*
================
rvWeaponMachinegun::State_DrumSpinUp

Spin the drum from a slow speed to a fast speed
================
*/
stateResult_t rvWeaponMachinegun::State_DrumSpinUp(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
	};
...
/*
================
rvWeaponMachinegun::State_DrumSpinDown

Spin the drum down from a faster speed
================
*/
stateResult_t rvWeaponMachinegun::State_DrumSpinDown(const stateParms_t& parms) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
	};
...
/*
================
rvWeaponMachinegun::Frame_ClaspOpen

Open the clasp that holds in the clips
================
*/
stateResult_t rvWeaponMachinegun::Frame_ClaspOpen(const stateParms_t& parms) {
...
================
*/
stateResult_t rvWeaponMachinegun::Frame_ClaspClose(const stateParms_t& parms) {
	if (mods & NAILGUN_MOD_ROF_AMMO) {
